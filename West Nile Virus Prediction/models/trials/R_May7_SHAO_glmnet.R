...
levels(factor(train$dMonth))
levels(factor(test$dMonth))

x=model.matrix(~dMonth+Species2+Latitude+Longitude+Block+WnvPresent-1, data=train)[,-6]
xtest=model.matrix(~dMonth05+dMonth+Species2+Latitude+Longitude+Block-1, data=test)[,-6]
x[1,]
xtest[1,]
...
p<-predict(fit, newx = xvalidation, s = 0, type = "response")
auc(yvalidation,p)
...
pSubmit<-predict(fitSubmit, newx = as.matrix(xtest),s = 0, type = "response")
