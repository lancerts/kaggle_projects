...
## prep the species column by moving the test-only UNSPECIFIED CULEX to CULEX PIPIENS, and re-doing the levels
## logistic regression will complain otherwise
vSpecies<-c(as.character(x$Species),as.character(test$Species))
vSpecies[vSpecies=="UNSPECIFIED CULEX"]<-"CULEX PIPIENS"
...
write.csv(submissionFile,"weighted_four_factors_PIPIENS.csv",row.names=FALSE,quote=FALSE)