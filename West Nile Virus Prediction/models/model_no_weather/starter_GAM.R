...
my.x = data.frame(x[,list(WnvPresent, dYear,dWeek, Species2, Latitude, Longitude,Block,NumMosquitos,AddressAccuracy)])
x1<-my.x[x$dYear!=2011,]
x2<-my.x[x$dYear==2011,]

## GAM modelling
require(gam)
fitCv = gam(WnvPresent ~ s(dWeek) + Species2 + lo(Latitude, Longitude)+s(Block)+s(dWeek):Species2, data = x1, family="binomial")
...
write.csv(submissionFile,"submitGAM1.csv",row.names=FALSE,quote=FALSE)
