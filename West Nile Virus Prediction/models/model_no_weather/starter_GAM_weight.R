...
my.x = data.frame(x[,list(WnvProb,WnvPresent,WnvNotPresent, dYear,dWeek,dMonth, Species2, Latitude, Longitude,Block,NumMosquitos,AddressAccuracy,TrapNumber)])
x1<-my.x[x$dYear!=2011,]
x2<-my.x[x$dYear==2011,]

## GAM modelling
require(gam)
fitCv = gam(cbind(x1$WnvPresent,x1$WnvNotPresent) ~ s(dWeek) + Species2 + lo(Latitude, Longitude)+s(Block)+s(dWeek):Species2+s(NumMosquitos), data = x1, family="binomial")
...
fitSubmit <- gam(WnvProb ~ s(dYear)+s(dWeek) + Species2 + lo(Latitude, Longitude)+s(Block)+s(dWeek):Species2+s(NumMosquitos), data = my.x, family="binomial",weight=my.x$NumMosquitos)
...
write.csv(submissionFile,"submission/GAM_weight.csv",row.names=FALSE,quote=FALSE)
