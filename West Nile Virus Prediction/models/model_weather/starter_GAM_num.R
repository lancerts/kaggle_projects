...
my.x = data.frame(x[,list(WnvProb, dYear,dWeek, Species2, Latitude, Longitude,Block,NumMosquitos,AddressAccuracy)])
x1<-my.x[x$dYear!=2011,]
x2<-my.x[x$dYear==2011,]

## GAM modelling
require(gam)
fitCv = gam(WnvProb ~ s(dWeek) + Species2 + lo(Latitude, Longitude)+s(Block)+s(dWeek):Species2+s(NumMosquitos), data = x1, family="binomial")
...
fitSubmit1 <- gam(WnvPresent ~ s(dYear)+s(dWeek) + Species2 + lo(Latitude, Longitude)+s(Block)+s(dWeek):Species2+s(NumMosquitos), data = my.x, family="binomial")
...
write.csv(submissionFile,"submission/GAMmean.csv",row.names=FALSE,quote=FALSE)

##
###
##
test <- fread("data/test3.csv")
## prep the species column by moving the test-only UNSPECIFIED CULEX to CULEX ERRATICUS, and re-doing the levels
## logistic regression will complain otherwise
vSpecies<-c(as.character(x$Species),as.character(test$Species))
vSpecies[vSpecies=="UNSPECIFIED CULEX"]<-"CULEX ERRATICUS"
...
colnames(submissionFile)<-c("Id","WnvPresent")
options("scipen"=100, "digits"=8)
write.csv(submissionFile,"submission/GAMmedian.csv",row.names=FALSE,quote=FALSE)