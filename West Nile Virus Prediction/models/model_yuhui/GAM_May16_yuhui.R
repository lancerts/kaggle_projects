...
fitSubmit <- gam(WnvPresent ~ s(Week) + Species + lo(Latitude, Longitude)+s(Block)+s(Week):Species+s(NumMosquitos) + s(Tavg) + s(DewPoint), data = train, family="binomial")
...
write.csv(submissionFile,"../submission/GAM_May16_yuhui.csv",row.names=FALSE,quote=FALSE)
