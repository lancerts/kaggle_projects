...
```{r}
weather.data <- read.csv("E:/Dropbox/kaggle/West Nile Virus Prediction/data/weather.csv", na.strings=c("M", "-", "", " "))
dim(weather.data)
str(weather.data)
head(weather.data)
```
...
write.csv(train, "E:/Dropbox/kaggle/West Nile Virus Prediction/data/train5.csv", row.names=F)
write.csv(test, "E:/Dropbox/kaggle/West Nile Virus Prediction/data/test5.csv", row.names=F)
...
nrow(test)
sort(test$NumMosquitos,decreasing = TRUE)[1:100]
write.csv(train, "data/train_weight.csv", row.names=F)
write.csv(test, "data/test_noAddress_trap.csv", row.names=F)